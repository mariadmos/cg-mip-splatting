...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_val : bool = False):
    with torch.no_grad():
        ...
        if not skip_val and len(scene.getValCameras()) > 0:
             render_set(dataset.model_path, "val", scene.loaded_iter, scene.getValCameras(), gaussians, pipeline, background, kernel_size, scale_factor=scale_factor)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.skip_val)