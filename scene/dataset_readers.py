...
class SceneInfo(NamedTuple):
    ...
    ply_path: str
    val_cameras: list = []   # held-out validation set (empty unless reader fills it)
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    if eval:
        # 3-way split (3DGS-adapted): every llffhold-th frame is held out, then
        # the held-out frames are dealt alternately into test / val. The rest is
        # train. With 180 frames & llffhold=8 -> ~157 train / ~12 test / ~11 val.
        train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
        holdout = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
        test_cam_infos = holdout[0::2]
        val_cam_infos = holdout[1::2]
    else:
        train_cam_infos = cam_infos
        test_cam_infos = []
        val_cam_infos = []

    print(f"Split: {len(train_cam_infos)} train / {len(val_cam_infos)} val / {len(test_cam_infos)} test")
