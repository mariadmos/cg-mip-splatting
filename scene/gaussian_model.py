...
class GaussianModel:
    ...
    def compute_3D_filter(self, cameras):
        # Ablacao do filtro 3D (novidade 1): com filter_3D = 0, get_scaling_with_3D_filter
        # devolve a escala crua e get_opacity_with_3D_filter tem coef = 1, ou seja, o filtro
        # 3D de suavizacao fica efetivamente desligado, sem tocar no resto do pipeline.
        if getattr(self, "disable_3D_filter", False):
            self.filter_3D = torch.zeros((self.get_xyz.shape[0], 1), device=self.get_xyz.device)
            return
    ...
    def save_ply(self, path):
        ...
        # Column-wise assignment instead of list(map(tuple, attributes)): the
        # latter builds millions of Python tuples and blows up CPU RAM (OOM on
        # dense scenes). This writes identical data with a fraction of the memory.
        for i, (name, _) in enumerate(dtype_full):
